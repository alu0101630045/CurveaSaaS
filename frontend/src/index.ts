...
const renderAnalysis = (data: PlotResponse): void => {
...
const getFiniteYValues = (points: Point[]): number[] =>
    points
...
const drawCurve = (plotData: PlotResponse): void => {
...
const solicitarPuntos = async (): Promise<void> => {
    const input = document.getElementById('funcionInput') as HTMLInputElement | null;
    if (!input) {
        return;
    }
    ...
    try {
        ...
        drawCurve(data as PlotResponse);
        renderAnalysis(data as PlotResponse);
